...
function wzjob_author_settings_page_html() {
                ...
                <form method="post" action="options.php" style="margin-top:20px;">
                    ...
                    <table class="form-table">
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="text" id="wzjob_org_logo" name="wzjob_author_options[org_logo]" value="<?php echo esc_attr($opts['org_logo'] ?? ''); ?>" class="regular-text">
                                <button class="button" id="wzjob_org_logo_upload_btn">Upload</button>
                                <div style="margin-top:10px;">
                                    <img id="wzjob_org_logo_preview" src="<?php echo esc_url($opts['org_logo'] ?? ''); ?>" style="max-width:120px;">
                                </div>
                            </td>
                        </tr>
    ...
    jQuery(document).ready(function($){
        ...
        $('#wzjob_org_logo_upload_btn').click(function(e){
            e.preventDefault();
            ...
            mediaUploader.on('select', function(){
                var attachment = mediaUploader.state().get('selection').first().toJSON();
                $('#wzjob_org_logo').val(attachment.url);
                $('#wzjob_org_logo_preview').attr('src', attachment.url);
            });
            mediaUploader.open();
        });
    });
...
// Rankmath SEO Settings - Important

add_filter('rank_math/json_ld', function($data, $jsonld) {
    foreach ($data as $key => $schema) {
        if (isset($schema['@type']) && in_array($schema['@type'], ['Article', 'NewsArticle', 'JobPosting'])) {
            $data[$key]['author'] = [
                "@id" => home_url('/#author')
            ];
        }
        if (is_front_page() && isset($schema['@type']) && $schema['@type'] === 'Person') {
            unset($data[$key]);
        }
        if (is_front_page() && isset($schema['@type']) && $schema['@type'] === 'NewsArticle') {
