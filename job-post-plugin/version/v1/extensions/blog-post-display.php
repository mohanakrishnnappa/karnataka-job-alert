...
add_shortcode( 'gmpipo_blog', 'gmpipo_blog_grid_display' );

function gmpipo_blog_grid_display( $atts ) {
    ...
    if ( empty( $categories ) ) {
        return '<p>No categories specified. Use: [gmpipo_blog categories="category-slug1,category-slug2"]</p>';
    }
    ...
    // Add inline CSS
    $output .= '<style>
        .gmpipo_blog_container {
            width: 100%;
            padding: 20px 0;
        }

        .gmpipo_blog_category_section {
            margin-bottom: 50px;
        }

        .gmpipo_blog_category_title {
            font-size: 28px;
            font-weight: bold;
            color: #333;
            margin-bottom: 25px;
        ...
        .gmpipo_blog_title:hover {
            color: #2874BF;
        }

        .gmpipo_blog_no_posts {
            grid-column: 1 / -1;
            padding: 30px;
            background: #f5f5f5;
            border-radius: 8px;
    ...
    $output .= '<div class="gmpipo_blog_container">';
    ...
    foreach ( $categories as $category_slug ) {
        ...
        $output .= '<div class="gmpipo_blog_category_section">';
        $output .= '<h1 class="gmpipo_blog_category_title">' . esc_html( get_the_title() ) . '</h1>';
        ...
        $output .= '<div class="gmpipo_blog_grid">';
        ...
        if ( $query->have_posts() ) {
            while ( $query->have_posts() ) {
                ...
                if ( $image_url ) {
                    $output .= '<div class="gmpipo_blog_card">';
                    $output .= '<a href="' . esc_url( $post_permalink ) . '" style="text-decoration: none;">';
                    $output .= '<img src="' . esc_url( $image_url ) . '" alt="' . $image_alt . '" class="gmpipo_blog_image">';
                    $output .= '</a>';
                    $output .= '<div class="gmpipo_blog_content">';
                    $output .= '<a href="' . esc_url( $post_permalink ) . '" style="text-decoration: none;">';
                    $output .= '<h3 class="gmpipo_blog_title">' . esc_html( get_the_title( $post_id ) ) . '</h3>';
                    $output .= '</a>';
                    $output .= '</div>';
                    $output .= '</div>';
                }
            ...
            $output .= '<div class="gmpipo_blog_no_posts">No posts found in this category.</div>';
        }
...
function blog_post_display_page() {
    add_submenu_page(
        ...
        'Blog Post Display',
        'manage_options',
        'graymarket-blog-post-display',
        'graymarket_blog_post_display_page_html'
    );
}

// Information page HTML
function graymarket_blog_post_display_page_html() {
