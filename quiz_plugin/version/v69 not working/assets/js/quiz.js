if (!document.querySelector(".wzq-wrapper")) {
    ...
    function startQuiz() {
        ...
        document.querySelector(".wzq-start-btn").addEventListener("click", async () => {

            // show quiz UI
            document.querySelector(".wzq-start-screen").style.display = "none";
            document.querySelector(".wzq-card").style.display = "block";

            // 1️⃣ get IDs
            const res = await fetch(wzq_ajax.url, {
                method: "POST",
            ...
            // 3️⃣ save
            sessionStorage.setItem("quiz_order", JSON.stringify(order));

            // 4️⃣ set total
            total = order.length;

            // 5️⃣ load first batch
            loadNextBatch();
        });
